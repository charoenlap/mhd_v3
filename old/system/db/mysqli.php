...
class db{
    ...
    public function query($sql) {
		...
		if (!$this->db->errno) {
			if ($query instanceof \mysqli_result) {
				...
				setLog('SUCCESS db query : '.$sql);

				return $result;
			} else {
				setLog('FAIL db query : '.$sql);
				return true;
			}
		} else {
			// echo $this->db->error;
			setLog('ERROR query : '.$sql);
	...
	public function get($table) {
		...
		$this->lastquery = $sql;
		if (!$this->db->errno) {
			setLog('SUCCESS db get(select) : '.$sql);
		} else {
			$result = false;
			setLog('ERROR db get(select) : '.$sql.' '.$this->db->error);
    ...
    public function update($table,$input,$where=''){
		...
		if (!$this->db->errno) {
			...
			setLog('SUCCESS db update : '.$this->lastquery);
		} else {
			$result = false;
			setLog('ERROR db update : '.$this->lastquery.' '.$this->db->error);
			trigger_error('Error: ' . $this->db->error  . '<br />Error No: ' . $this->db->errno . '<br />' . $update);
		}
	...
	public function insert($table,$input){
		...
		if (!$this->db->errno) {
			...
			setLog('SUCCESS db insert : '.$this->lastquery);
		} else {
			$result = false;
			setLog('ERROR db insert : '.$this->lastquery.' '.$this->db->error);
	...
	public function delete($table,$where=''){
		if (!empty($where)||!empty($this->where)) {
			...
			if (!$this->db->errno) {
				$result = ($query?true:false);
				setLog('SUCCESS db delete : '.$this->lastquery);
			} else {
				$result = false;
				setLog('ERROR db delete : '.$this->lastquery.' '.$this->db->error);
				trigger_error('Error: ' . $this->db->error  . '<br />Error No: ' . $this->db->errno . '<br />' . $delete);
			}
			$this->clean();
	        return $result;
		}
