...
class Report extends CI_Controller
{
  ...
  public function index($year)
  {
    ...
    $data['heading_title'] = 'แจ้งส่งผลการทดสอบ '.$year_info->year;
    ...
    $year_config = $this->model_setting->get('config_register_year_id');
    if($year_config == $year_id){
      $data['year_config'] = 1;
    } else {
      $data['year_config'] = 0;
    }
    ...
    $program_sub = $this->model_register_program->getListProgramBySub($register_id, $member_id, $company_id, $year_id);
  ...
  public function program($slug, $year_id, $register_program_id = null)
  {
    $data = array();
    $data['action'] = base_url('report/graph');

    // ปรับใหม่จับจาก slug เดิมเอา slug มาเปิดตามชื่อไฟล์เปลี่ยนเอา slug มาเป็น id
    // $program_info = $this->model_program->getListsSlug($slug);
    ...
    $trial = $this->model_trial->getLists($filter, '', '', 'id', 'desc');
  ...
  public function trial($program_slug, $trial_slug, $register_program_id = null)
  {
    ...
    // $year_id = $this->model_setting->get('config_register_year_id'); // year
    $year_id = $trial_info->year_id; // year
    ...
    $data['tools_auto'] = $this->model_program_tool->getLists($filter, 0, 9999999999, 'name', 'ASC');

    // only eqac
    $filter = array('program_id' => $program_id, 'section' => 1);
    $data['tools_sec1'] = $this->model_program_tool->getLists($filter, 0, 9999999999,'name','ASC');
    $filter = array('program_id' => $program_id, 'section' => 2);
    $data['tools_sec2'] = $this->model_program_tool->getLists($filter, 0, 9999999999,'name','ASC');
    $filter = array('program_id' => $program_id, 'section' => 3);
    $data['tools_sec3'] = $this->model_program_tool->getLists($filter, 0, 9999999999,'name','ASC');
    // ? ========== initial loading data ==========
    ...
    $has_olddata = false;
    $report_id = 0;
    $data['value_status'] = "show"; // กำหนดซ่อนเฉพาะ
    $filter = array(
    ...
    if (count($report_info) > 0) {
      ...
      if($program_id !== '9' || $program_id !== '7' || $program_id !== '10' || $program_id !== '12' || $program_id !== '13'){
        $filter_old = array(
          'program_id'  => $program_id,
          'member_id'   => $member_id,
          'register_id' => $register_id,
        ...
        $report_info_old = $this->model_report->getReportOne($filter_old);
        $data['save'] = json_decode($report_info_old->save, true);
        $data['value_status'] = "hide"; // กำหนดซ่อนเฉพาะ
      }
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      // ถ้ามีการกด submit จะให้ดึง data มา่โชว์
      $data['value_status'] = "show";
      // ถ้ามีการกด submit จะให้ดึง data มา่โชว์


      if ($this->input->post('type') == 'preview') {
        if (!empty($_FILES)) {
          $config['upload_path'] = './upload/';
          $config['allowed_types'] = 'jpeg|jpg|png|pdf|tiff|bmp';
          ...
          foreach ($_FILES as $key => $value) {
            if (!empty($value['tmp_name']) && $value['size'] > 0) {
              if ($this->upload->do_upload($key)) {
                ...
                $data['file_path'] = base_url('upload/');
              } else {
                $this->session->set_userdata('uploadFailed', 'เกิดข้อผิดพลาดไม่สามารถอัพโหลดรูปภาพได้กรุณาลองใหม่อีกครั้ง ' . $this->upload->display_errors());
                redirect('report/program/' . $program_slug . '/'. $year_id);
              } // End else
            }
          }
        }
      ...
      if ($this->input->post('type') == 'submit') {
        ...
        redirect('report/program/' . $program_slug . '/'. $year_id .'/'.(!$main ? '/' . $register_program_id : ''));
      }
      // ? ========== Save to db ==========
      ...
      if (!empty($data['save']['report_date'])) {
        $data['report_date'] = $data['save']['report_date'];
      }else{
    ...
    // แก้ไขเรื่องการ show data เก่าและใหม่
    // $data['value_status'] = "hide";
    // $filter_old = array(
    //   'program_id'  => $program_id,
    //   'member_id'   => $member_id,
    //   'register_id' => $register_id,
    ...
    // );
    // $report_info_old = $this->model_report->getReportOne($filter_old);
    // $trial_id_old = $report_info_old['0']->trial_id;
    // $trial_id_new = $trial_id;
    // if($trial_id_old == $trial_id_new){
    //   $data['value_status'] =  "show";
    // }
    //  ถ้ายังไม่มีการ save report ใหม่จะดึงของเก่ามา show 
    // if(empty($data['save'])){
    //   $filter_old = array(
    //     'program_id'  => $program_id,
    //     'member_id'   => $member_id,
    ...
    //     'company_id'  => $company_info->id
    //   );
    //   $report_info_old = $this->model_report->getReport($filter_old);
    //   $data['value_status'] = "hide";
    //   $trial_id_old = $report_info_old['0']->trial_id;
    //   $trial_id_new = $trial_id;
    //   if($trial_id_old == $trial_id_new){
    //     $data['value_status'] =  "show";
    //   }
    //   foreach ($report_info_old as $key => $value) {
        
    //   }
    //   $data['save'] = json_decode($value->save, true);
    //   $data['result_remove'] = true;
    // }
