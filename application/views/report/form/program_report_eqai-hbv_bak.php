...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        const setRequiredInputText = (thiselement, value, key) => {
            if (value.length > 0) {
                $(thiselement).parent('td').parent('tr').next('tr').children('td.instrument_'+key).children('input').attr('required', 'required');
                let sum = key+1;
                $('.fixed-select-'+sum).attr('required','required');
                // $(thiselement).parent('td').parent('tr').next('tr').children('td:eq(1)').children('input').attr('required', 'required');
                // $(thiselement).parent('td').parent('tr').next('tr').next('tr').children('td:eq(1)').children('input').attr('required', 'required');
                // $(thiselement).parent('td').parent('tr').next('tr').next('tr').next('tr').children('td:eq(1)').children('input').attr('required', 'required');
            } else {
                $(thiselement).parent('td').parent('tr').next('tr').children('td.instrument_'+key).children('input').removeAttr('required');
                let sum = key+1;
                $('.fixed-select-'+sum).removeAttr('required');
                // $(thiselement).parent('td').parent('tr').next('tr').children('td:eq(1)').children('input').removeAttr('required');
                // $(thiselement).parent('td').parent('tr').next('tr').next('tr').children('td:eq(1)').children('input').removeAttr('required');
                // $(thiselement).parent('td').parent('tr').next('tr').next('tr').next('tr').children('td:eq(1)').children('input').removeAttr('required');
            }
        }
        const setRequiredSpecimen = (value, key) => {
            if (value.length > 0) {
                $('.fixed-select-'+key).removeAttr('required');
                let sum = key-1;
                $('.check_select_'+sum).attr('required','required');
            }else{
                $('.fixed-select-'+key).attr('required','required');
                let sum = key-1;
                $('.check_select_'+sum).removeAttr('required');
        ...
        const requiredInputInstrument = (selectElement, isRequired = false) => {
            ...
            if (isRequired) {
                inputInstrument_1.attr('required', 'required');
                // inputInstrument_2.attr('required', 'required');
                // inputInstrument_3.attr('required', 'required');
            } else {
                // inputInstrument.removeAttr('required');
                inputInstrument_1.removeAttr('required');
            }
        }
