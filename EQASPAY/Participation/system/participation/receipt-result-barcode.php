<?php
	...
	$sql = "SELECT * FROM participantRegister WHERE (eqacp='$doc' OR eqahp='$doc' OR eqatp='$doc' OR eqapp='$doc' OR beqamp='$doc' OR heqamp='$doc' OR uceqamp='$doc' OR eqaisypp='$doc' OR eqaihbvp='$doc' OR eqabgramp='$doc' OR eqabafbp='$doc' OR eqabidenp='$doc') ORDER BY id ASC;";
	
	$result = mysqli_query($link, $sql);
	$rowcount = mysqli_num_rows($result);
	
	$sql2 = "SELECT * FROM paymentSlip WHERE doc = '$doc'";
	$result2 = mysqli_query($link, $sql2);
	$dbrr2 = mysqli_fetch_array($result2);
	$mail = $dbrr2["staff"];
	
	
	$sql3 = "SELECT * FROM staff WHERE email = '$mail'";
...
<table width="100%" border="0" >
  <tbody>
    <tr>
      <td width="20%"><img src="../../tools/images/home/MT-TH-MU-H.png" width="200px"></td>
      <td style="text-align:right" class="h6">DOC NO: 65/<?php echo(str_pad($doc, 3, '0', STR_PAD_LEFT));  ?></td>
    </tr>
  </tbody>
</table>

<center><h7 class="ft-sarabun">แบบสรุปรายการออกใบเสร็จรับเงินค่าธรรมเนียมการสมัครสมาชิก ประจำปี 2565</h7></center>
...
<?php	
	...
	?>
	  ...
	  <tr align="center">
		 <td><?php echo $i; ?></td>
	  <td align="left"><span style="font-family: 'Libre Barcode 39 Text', cursive; font-size: 35pt;"><?php echo "*".(($row["id"])-20220000)."*";?></span><input type="hidden" name="id<?php echo $i; ?>" value="<?php echo $row["id"];?>"</th></td>
	<td align="left"><?php echo $row["instituteName"];?></td>
	<td><?php if($row["eqacp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";$c = 3000;}?></td>
	<td><?php if($row["eqahp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";$h = 4500;}?></td>
	<td><?php if($row["eqatp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";$t = 4500;}?></td>
	<td><?php if($row["eqapp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";$p = 2000;}?></td>
	<td><?php if($row["beqamp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";$mb = 2000;}?></td>
	<td><?php if($row["heqamp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";$mh = 1500;}?></td>
	<td><?php if($row["uceqamp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";$mu = 1000;}?></td>
	<td><?php if($row["eqaisypp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";$is = 2200;}?></td>
	<td><?php if($row["eqaihbvp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";$ih = 2200;}?></td>
	<td><?php if($row["eqabgramp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";}?></td>
	<td><?php if($row["eqabafbp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";}?></td>
	<td><?php if($row["eqabidenp"]==$doc) { echo "<i class='fas fa-check-circle'></i>";}?></td>
	<td><?php 
...
<table width="100%"  border="0" style="font-size:10pt" class="ft-sarabun" cellpadding="0px">
  <tbody>
    <tr>
      <td width="10%" style="text-align: right; margin-top: 50px; vertical-align: bottom">จัดเตรียมโดย</td>
      <td  width="30%"><div style="border-bottom:dotted;border-bottom-width: 1px; padding-top: 10px; margin-top: 50px"><center><img class="mb-2" src="images/sign/<?php echo $dbrr3["nameEN"] ?>.png" height="50pt"></center></div></td>
		<td width="20%"></td>
	 <td width="10%" style="text-align: right; margin-top: 50px; vertical-align: bottom">ออกใบเสร็จโดย</td>
		 <td width="30%"><div style="border-bottom:dotted;border-bottom-width: 1px; padding-top: 10px; margin-top: 50px; height: 50pt;"></div></td>
    </tr>
	  
    <tr>
       <td style="text-align:right"> </td>
		<td><div style="text-align: center;border-bottom:dotted;border-bottom-width: 1px;padding-top: 10px;margin-top: 10px"><?php echo $dbrr3["nameTH"]." ".$dbrr3["surNameTH"]; ?></div></td>
		<td></td>
		<td style="text-align:right"> </td>
       ...
       <td style="text-align:right">ตำแหน่ง </td>
		<td style="text-align: center;border-bottom:dotted;border-bottom-width: 1px;padding-top: 10px;"><?php echo $dbrr3["position"]; ?></td>
		<td></td>
		<td style="text-align:right">ตำแหน่ง </td>
    ...
    <tr>
      <td style="text-align:right">วันที่</td>
		<td style="text-align: center;border-bottom:dotted;border-bottom-width: 1px; padding-top: 10px; margin-top: 50px"> <?php
			$date = $dbrr2["date"];
			echo thai_date_fullmonth(strtotime($date));
...
<div  class="divFooter ft-sarabun">
          <span style="font-size: 8pt !important;">F/QP043-SP-01/07 แก้ไขครั้งที่ 0 (1 มิ.ย. 64) </span>
          </div>
...
<center>
          <button type="button" style="width: 60%; height: 100px; font-size: 30pt" class="btn btn-success  mr-5" onClick="window.print()">
